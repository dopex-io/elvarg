...
import { OptionTokenInfoInterface, OlpContext } from 'contexts/Rolp';
...
const Stats = () => {
    ...
    return (
      <Box className="border rounded border-transparent p-2 ml-3">
        ...
        {zipWith(
          ...
          strikes,
          optionTokenPrices,
          function (info, strike, price) {
            const liq = price
              .mul(info.tokenLiquidity)
              .mul(
                getContractReadableAmount(tokenPrice, DEFAULT_TOKEN_DECIMALS)
              )
              .div(oneEBigNumber(2 * DEFAULT_TOKEN_DECIMALS));
            return (
              <Box className="flex justify-between">
                ...
                <Typography variant="h6" className="text-white text-right mt-3">
                  $
                  {formatAmount(
                    getUserReadableAmount(liq, DEFAULT_TOKEN_DECIMALS)
                  )}
                </Typography>
              </Box>
            );
  ...
  const handleIsPut = useCallback(
    ...
    [setSelectedIsPut, updateOlpEpochData]
  );
    ...
    <Box className="flex">
      ...
      <Box className="flex">
        ...
        {getTotalLiquidityProvidedBox(
          ...
          olpEpochData?.strikeTokenPrices!,
          tokenPrice
        )}
      </Box>
    </Box>
