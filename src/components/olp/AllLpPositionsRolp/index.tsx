...
import { LpPositionInterface, OlpContext } from 'contexts/Rolp';
...
const AllLpPositions = () => {
  ...
  const filterSortStrike = useMemo(() => {
    ...
    return filteredPositions.sort(
      (a, b) => b.p.markup.toNumber() - a.p.markup.toNumber()
    );
                ...
                ?.map((p, idx) => {
                  return (
                    <AllPositionsTable
                      ...
                      numTokensProvided={p.p.numTokensProvided}
                      numTokensSold={p.p.numTokensSold}
                      markup={p.p.markup}
                      usdReceived={p.p.usdReceived}
