...
const PositionsTable = () => {
  ...
  const { signer, optionPerpUserData, optionPerpData, optionPerpEpochData, updateOptionPerpUserData, updateOptionPerp, updateOptionPerpEpochData } =
    useBoundStore();
  ...
  const handleExercise = useCallback(
    ...
    [optionPerpUserData, optionPerpData, optionPerpEpochData, updateOptionPerpUserData, updateOptionPerp, updateOptionPerpEpochData, signer, sendTx]
  );
  ...
  return (
    <Box>
        ...
        <Table className="rounded-xl">
          <TableHead className="rounded-xl">
            <TableRow>
              ...
              <TableHeader label="Strike" />
              <TableHeader label="PnL" />
              <TableHeader label="Action" variant="text-end" />
            </TableRow>
          </TableHead>
          <TableBody className="rounded-lg">
            {optionPerpUserData?.optionPositions?.map((position, i) => (
              <TableRow key={i}>
                ...
                <TableCell className="pt-1 border-0">
                  <Typography variant="h6" className="text-[#6DFFB9]">
                    ${getUserReadableAmount(position.strike, 8).toFixed(2)}
                  </Typography>
                </TableCell>
                <TableCell className="pt-1 border-0">
                ...
                <TableCell className="flex justify-end border-0">
                  <CustomButton
                    ...
                    color={
                      position?.canBeSettled ? 'mineshaft' : 'primary'
                    }
                    disabled={
      ...
      <Box className="flex">
        {optionPerpUserData?.optionPositions?.length === 0 ? (
