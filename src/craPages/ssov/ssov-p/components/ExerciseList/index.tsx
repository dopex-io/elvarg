...
import { SsovProperties, SsovContext } from 'contexts/SsovPuts';
...
const ExerciseList = ({
  ...
  const { epochStrikeTokens, userEpochStrikeDeposits, userEpochPutsPurchased } =
    ...
    (async function () {
      ...
      const userExercisableOptions = epochStrikes.map((strike, strikeIndex) => {
        ...
        const purchasedAmount = getUserReadableAmount(
          userEpochPutsPurchased[strikeIndex],
          18
        );
        ...
        const pnlAmount = settlementPrice.isZero()
          ? tokenPrice
              .sub(strike)
              .mul(userEpochPutsPurchased[strikeIndex])
              .div(tokenPrice)
          : settlementPrice
              .sub(strike)
              .mul(userEpochPutsPurchased[strikeIndex])
