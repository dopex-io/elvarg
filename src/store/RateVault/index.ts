...
export const createRateVaultSlice: StateCreator<
  RateVaultSlice & WalletSlice & CommonSlice,
...
> = (set, get) => ({
  ...
  updateRateVaultUserData: async () => {
    ...
    set((prevState: { isLoading: boolean }) => ({
      ...prevState,
      isLoading: true,
    }));
    ...
    for (let i in rateVaultEpochData.callsLeverages) {
      for (let j in rateVaultEpochData.putsLeverages) {
        ...
        rateVaultEpochData.epochStrikes.map(
          (strike: BigNumber, strikeIndex: number) => {
    ...
    set((prevState: { rateVaultData?: RateVaultData }) => ({
      ...prevState,
      rateVaultUserData: {
        userEpochStrikeDeposits,
        userStrikePurchaseData,
